...
namespace MVXUnity
{
    public class MvxAudioChunksPool
    {
        ...
        public MvxAudioChunk AllocateAudioChunk(float[] data, UInt32 channelsCount, UInt32 sampleRate, UInt32 frameNr)
        {
            lock(m_threadLock)
            {
                ...
                if (m_unusedAudioChunks.Count > 0)
                {
                    ...
                    audioChunk.Reset(data, channelsCount, sampleRate, frameNr);
                }
                else
                {
                    audioChunk = new MvxAudioChunk(data, channelsCount, sampleRate, frameNr);
                }
        ...
        public MvxAudioChunk AllocateAudioChunk(byte[] data, UInt32 bytesPerSample, UInt32 channelsCount, UInt32 sampleRate, UInt32 frameNr)
        {
            lock (m_threadLock)
            {
                MvxAudioChunk audioChunk = null;
                ...
                {
                    audioChunk = m_unusedAudioChunks.Dequeue();
                    audioChunk.Reset(data, bytesPerSample, channelsCount, sampleRate, frameNr);
                }
                else
                {
                    audioChunk = new MvxAudioChunk(data, bytesPerSample, channelsCount, sampleRate, frameNr);
                }
